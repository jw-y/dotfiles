...
Accounts live in `~/.codex-profiles/<name>`, holding credentials and profile-local SQLite databases. Shareable config, transcript files, and the binary/plugin caches live once in `~/.codex-profiles/.store`, which is not an account and is symlinked into each one. SQLite deliberately stays profile-local: sharing a main database file through a symlink separates it from its WAL/SHM files and can corrupt the database. `cdx link` rebuilds only the links that are safe to share.
