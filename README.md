...
cdx share-sqlite       # one safe SQLite directory for every account on this host
...
Accounts live in `~/.codex-profiles/<name>` and hold credentials plus account-local state. Shareable config, transcripts, SQLite state, and binary/plugin caches live once per host. Ordinary files and directories are linked from `~/.codex-profiles/.store`; SQLite is shared through Codex's supported `sqlite_home` setting so each database stays beside its WAL/SHM files. `cdx share-sqlite` performs that migration and archives old per-profile files instead of deleting them. Its default destination is `.store`; on an NFS-home host, pass an absolute host-local directory such as `/var/tmp/$USER/codex-sqlite`.
